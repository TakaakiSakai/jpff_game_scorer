...
function App({ signOut, user }: WithAuthenticatorProps) {
  ...
  async function load() {
    ...
    setTeams(t.data)
    const g = await client.models.Game.list({ limit: 200, authMode: 'userPool' })
    setGames(g.data)
  }
  useEffect(() => { load() }, [])

  // 合計計算（未入力は0）
  const calc = (g: Schema['Game']['type']) => {
    const s = (n?: number) => n ?? 0
    const h = s(g.q1Home)+s(g.q2Home)+s(g.q3Home)+s(g.q4Home)+s(g.otHome)
    const a = s(g.q1Away)+s(g.q2Away)+s(g.q3Away)+s(g.q4Away)+s(g.otAway)
    return { h: g.finalHome ?? h, a: g.finalAway ?? a }
  }
  ...
  return (
    <div style={{maxWidth:900, margin:'0 auto', padding:16}}>
      <header style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        ...
        <div><span style={{marginRight:12}}>{user?.username}</span><button onClick={signOut}>Sign out</button></div>
      </header>
      ...
      <section>
        ...
        <table style={{width:'100%', marginTop:8, borderCollapse:'collapse'}}>
          <thead><tr><th>Date</th><th>Home</th><th>Away</th><th>Score</th><th>Venue</th></tr></thead>
          <tbody>
            {games.map(g=>{
              const home = teams.find(t=>t.id===g.homeTeamID)?.name || ''
              const away = teams.find(t=>t.id===g.awayTeamID)?.name || ''
              const totals = calc(g)
              return <tr key={g.id}><td>{g.date}</td><td>{home}</td><td>{away}</td><td>{totals.h} - {totals.a}</td><td>{g.venue ?? ''}</td></tr>
            })}
          </tbody>
        </table>
      </section>
...
function TeamQuickAdd({ onAdded }: { onAdded: ()=>void }) {
  const [name, setName] = useState(''); const [division, setDivision] = useState('')
  async function add() {
    if(!name) return
    await client.models.Team.create({ name, division }, { authMode: 'userPool' })
...
function GameQuickAdd({ teams, onAdded }: { teams: Schema['Team']['type'][], onAdded: ()=>void }) {
  const today = new Date().toISOString().slice(0,10)
  const [date, setDate] = useState<string>(today)
  const [venue, setVenue] = useState(''); const [homeTeamID, setHomeTeamID] = useState(''); const [awayTeamID, setAwayTeamID] = useState('')
  const [q, setQ] = useState({q1H:0,q1A:0,q2H:0,q2A:0,q3H:0,q3A:0,q4H:0,q4A:0,otH:0,otA:0})

  function step(key: keyof typeof q, d: number) { setQ(prev => ({...prev, [key]: Math.max(0, (prev[key] as number)+d)})) }

  async function add() {
    if(!date || !homeTeamID || !awayTeamID) return
    await client.models.Game.create({
      date, venue, homeTeamID, awayTeamID,
      q1Home:q.q1H, q1Away:q.q1A, q2Home:q.q2H, q2Away:q.q2A,
      q3Home:q.q3H, q3Away:q.q3A, q4Home:q.q4H, q4Away:q.q4A,
  ...
  const pairs = [['Q1 Home','q1H'],['Q1 Away','q1A'],['Q2 Home','q2H'],['Q2 Away','q2A'],['Q3 Home','q3H'],['Q3 Away','q3A'],['Q4 Home','q4H'],['Q4 Away','q4A'],['OT Home','otH'],['OT Away','otA']] as const
  ...
  return (
    <section style={{margin:'16px 0'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8, maxWidth:640}}>
        ...
        <div><label>Home</label><br/>
          <select value={homeTeamID} onChange={e=>setHomeTeamID(e.target.value)}>
            <option value="">選択</option>{teams.map(t=> <option key={t.id} value={t.id}>{t.name}</option>)}
          </select></div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:8, marginTop:8}}>
        {pairs.map(([label,key])=>(
          <div key={key}><label>{label}</label><br/>
